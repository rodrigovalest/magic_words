...
if ($_SERVER["REQUEST_METHOD"] === "GET") {
    ...
    if (!$weekly && $mode != "any") {
        $sql = "SELECT * FROM matches WHERE mode = '$mode' ORDER BY score DESC LIMIT 50";
    ...
    if ($weekly && $mode == "any") {
        $sql = "SELECT * FROM matches WHERE WEEK(datetime) = WEEK(NOW()) ORDER BY score DESC LIMIT 50";
        $result = mysqli_query($conn, $sql);    
    ...
    if ($weekly && $mode != "any") {
        $sql = "SELECT * FROM matches WHERE mode = '$mode' AND WEEK(datetime) = WEEK(NOW()) ORDER BY score DESC LIMIT 50";
